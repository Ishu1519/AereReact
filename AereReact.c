...
/* ================================================================
 * SHELL ATTACH
 * Confirmed working on Win11 25H2 build 26200.8246:
 *   - No WorkerW exists on this build
 *   - Parent directly into Progman
 *   - WS_CHILD window sits below SHELLDLL_DefView (icons stay visible)
 * ================================================================ */
static HWND attachToShell(void) {
    HWND progman = FindWindowW(L"Progman", NULL);
    if (!progman) return NULL;

    /* Attempt to spawn WorkerW — harmless if build ignores it */
    DWORD_PTR r = 0;
    SendMessageTimeoutW(progman, 0x052C, 0xD, 0x1, SMTO_NORMAL, 1000, &r);
    SendMessageTimeoutW(progman, 0x052C, 0x0, 0x0, SMTO_NORMAL, 1000, &r);
    Sleep(150);

    /* Check if WorkerW appeared */
    HWND workerW = FindWindowExW(NULL, NULL, L"WorkerW", NULL);
    while (workerW) {
        HWND dv = FindWindowExW(workerW, NULL, L"SHELLDLL_DefView", NULL);
        if (!dv) return workerW; /* WorkerW without icons = background layer */
        workerW = FindWindowExW(NULL, workerW, L"WorkerW", NULL);
    }

    /* No background WorkerW found — use Progman directly (26200 path) */
    return progman;
}
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show) {
    ...
    HWND shell = attachToShell();

    /* Get actual client dimensions of shell parent */
    if (shell) {
        RECT rc;
        GetClientRect(shell, &rc);
        if (rc.right  > 0) g_W = rc.right;
        if (rc.bottom > 0) g_H = rc.bottom;
    }
    ...
    g_hwnd = CreateWindowExW(
        ...
        0, 0, g_W, g_H,
        shell, NULL, hInst, NULL
    );

    if (!g_hwnd) return 1;

    /* Sit below icons in Z-order */
